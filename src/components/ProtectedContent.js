...
function getAllowedSections(role) {
  switch (role) {
    ...
    case "UserRoles.REPORT_ADMIN":
      return ["notifications", "waterReport", "history", "AccountingPost"];
  ...
  return (
    <div className="sidebar">
      ...
      {allowedSections.includes("Water") && (
        <button onClick={() => setActiveSection("Water")}>
      ...
      {allowedSections.includes("paymentCalc") && (
        <button onClick={() => setActiveSection("paymentCalc")}>
          Расчет суммы оплаты
        </button>
      )}
      ...
      {allowedSections.includes("wasteWater") && (
        <button onClick={() => setActiveSection("wasteWater")}>
          Учет объема сброса сточных вод
        </button>
      )}
      {allowedSections.includes("AccountingPost") && (
        <button onClick={() => setActiveSection("AccountingPost")}>
          Пункт учета
        </button>
      )}
      {allowedSections.includes("organizationInfo") && (
        <button onClick={() => setActiveSection("organizationInfo")}>
      ...
      {allowedSections.includes("AdminPanel") && (
        <button onClick={() => setActiveSection("AdminPanel")}>
          Админ панель
        </button>
      )}
      {allowedSections.includes("UserPage") && (
        <button onClick={() => setActiveSection("UserPage")}>
          О пользователях
        </button>
      )}
