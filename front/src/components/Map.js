...
const TableCell_status  = styled.td`
text-align: center;
font-size: 1.55rem;
padding: 0px;
    ...
    const DGisMap = () => {
      ...
      const filteredMarkers = useMemo(() => {
        ...
        .filter((m) => {
          ...
          const yearMatch = !yearFilter || m.start_date?.startsWith(yearFilter);
      ...
      const uniqueYears = useMemo(() => {
        return [...new Set(
          markers
          .map(m => m.start_date?.slice(0, 4)) // берём первые 4 символа
          .filter(Boolean) // убираем undefined/null/пустое
        )].sort();
      }, [markers]);

      useEffect(() => {
        console.log("Метки с датами:", markers.map(m => m.start_date));
      }, [markers]);
      ...
      return (
        ...
        {filteredMarkers.map((m, i) => (
          ...
          <TableCell_status status={m.status}>
