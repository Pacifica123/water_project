...
function CombinedRatesCoefs() {
  ...
  const getDateLimits = () => {
    ...
    switch (dateMode) {
      ...
      case "current":
        // ✔ можно выбрать сегодня и все предыдущие дни текущего месяца
  ...
  const getLatestRates = () => {
    ...
    rates.forEach((r) => {
      if (new Date(r.start_date) <= today) {
        ...
        if (!map[key] || new Date(r.start_date) > new Date(map[key].start_date))
          map[key] = r;
      }
    });
    return map;
  ...
  const getLatestCoefs = () => {
    ...
    coefs.forEach((c) => {
      if (new Date(c.start_date) <= today) {
        ...
        if (!map[key] || new Date(c.start_date) > new Date(map[key].start_date))
          map[key] = c;
      }
    });
    return map;
    ...
    <div className="combined-container" style={{ display: "flex", gap: "10px" }}>
        ...
        <div className="rates-container">
          ...
          {Object.entries(latestRates).length === 0 ? (
            ...
            Object.entries(latestRates).map(([k, r]) => (
              <div key={k}>
                <b>{formatRateType(k)}</b>: {r.value} руб. (с{" "}
          ...
          {hasFutureRate && (
            <p style={{ color: "red" }}>Есть будущая ставка</p>
          )}
          ...
          {rateAlertVisible && (
            <div className="custom-alert">✅ Ставка успешно добавлена!</div>
          )}
              ...
              <div className="modal">
                ...
                <form onSubmit={submitRate}>
                  ...
                  <div className="modal-buttons">
                    <button className="modal-edit-button" type="submit">
                      Сохранить
                    </button>
                    <button
      ...
      <section className="water-Rates-form">
        <div className="upcoef-container">
          ...
          <div>
            <div>
              <b>{formatCoefType("other_method")}:</b>{" "}
              {latestCoefs["other_method"]?.value ?? "-"} (
              {latestCoefs["other_method"]?.start_date
                ? formatDate(latestCoefs["other_method"].start_date)
                : "-"}
              )
            </div>
            <div>
              <b>{formatCoefType("out_permission")}:</b>{" "}
              {latestCoefs["out_permission"]?.value ?? "-"} (
              {latestCoefs["out_permission"]?.start_date
                ? formatDate(latestCoefs["out_permission"].start_date)
                : "-"}
              )
            </div>
          </div>
          ...
          {hasFutureCoef && (
            <p style={{ color: "red" }}>Есть будущий коэффициент</p>
          )}
